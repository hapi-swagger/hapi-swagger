 ...
 * @return {Object}
 */
internals.properties.prototype.parseProperty = function (name, joiObj, parameterType, useDefinitions, isAlt) {
    ...
    // get name
    if (!name && Utilities.geJoiLabel(joiObj)) {
    ...
    if (property.type === 'object') {
        if (Utilities.hasJoiChildren(joiObj)) {
            ...
            property = this.parseObject(property, joiObj, name, useDefinitions, isAlt);

            if (useDefinitions === true) {
                let objectSchema = Schema.build(property, 'object');
                let refName = this.definitions.append(name, objectSchema, this.getDefinitionCollection(isAlt), this.settings);
                property = { '$ref': this.getDefinitionRef(isAlt) + refName };
            }
    ...
    if (property.type === 'array') {
        ...
        if (useDefinitions === true) {
            let objectSchema = Schema.build(property, 'array');
            let refName = this.definitions.append(name, objectSchema, this.getDefinitionCollection(isAlt), this.settings);
            property = { '$ref': this.getDefinitionRef(isAlt) + refName };
        }
    }
 ...
 * @return {Object}
 */
internals.properties.prototype.parseObject = function (property, joiObj, name, useDefinitions, isAlt) {
    ...
    joiObj.forEach((obj) => {
        ...
        property.properties[itemName] = this.parseProperty(itemName, joiChildObj, null, useDefinitions, isAlt);
    });
    return property;
...
internals.properties.prototype.parseArray = function (property, joiObj, name, parameterType, useDefinitions, isAlt) {
    ...
    if (arrayItem) {
        ...
        //null, joiObj, parameterType, useDefinitions, isAlt
        let arrayItemProperty = this.parseProperty(itemName, arrayItem, null, useDefinitions, isAlt);
...
internals.properties.prototype.parseAlternatives = function (property, joiObj, name, parameterType, useDefinitions) {
    ...
    if (Hoek.reach(joiObj, '_inner.matches.0.schema')) {
        ...
        property = this.parseProperty(childName, child, parameterType, useDefinitions, false);
        ...
        if (this.settings.xProperties === true) {
            let altArray = joiObj._inner.matches.map((obj) => {
                ...
                return this.parseProperty(altName, obj.schema, parameterType, useDefinitions, true);
            });
        ...
        if (this.settings.xProperties === true) {
            let altArray = joiObj._inner.matches
                ...
                .map((joiNewObj) => {
                    ...
                    return this.parseProperty(altName, joiNewObj, parameterType, useDefinitions, true);
                })
                .filter((obj) => obj);
            property['x-alternatives'] = Hoek.clone(altArray);
        }
