...
internals.parseArray = function (property, joiObj, name, definitionCollection, type) {
    ...
    if (firstInclusionType) {
        ...
        if (name === 'items' && Array.isArray(joiObj._inner.inclusions[0]._meta)) {
            //name = Utilities.getJoiMetaProperty(joiObj._inner.inclusions[0], 'name');
            name = Hoek.reach(joiObj._inner.inclusions[0], '_settings.language.label');
        }

        let arrayProperty = properties.parseProperty(name, firstInclusionType);
