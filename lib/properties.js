...
properties.parseProperty = function (name, joiObj, definitionCollection, altDefinitionCollection, type, isAlt) {
    ...
    if (property.type === 'object') {
        if (Utilities.hasJoiChildren(joiObj)) {
            ...
            if(name){
               // console.error('ERROR: Joi.object() from "' + name + '" has no child properties. The resulting swagger.json will invalid against OpenAPI spec');
            }
        }
    }
...
internals.parseObject = function (property, joiObj, name, definitionCollection, altDefinitionCollection, isAlt) {
    ...
    // not valid object is array has no items
 //   if (!property.properties && !property.schema) {
 //       property = { type: 'void' };
 //       console.log('Error: object named "' + name + '" had no properties/schema and is invalid in OpenAPI');
 //   }
...
internals.parseAlternatives = function (property, joiObj, name, definitionCollection, altDefinitionCollection, type) {
    ...
    // convert .when() alternatives structures
        ...
        let altArray = joiObj._inner.matches
            ...
            .map((joiObj) => {
                let altName = (Utilities.geJoiLabel(joiObj) || name);
                return properties.parseProperty(altName, joiObj, definitionCollection, altDefinitionCollection, type, true);
            })
            .filter((obj) => obj);
