...
internals.properties.prototype.parseString = function (property, joiObj) {
  ...
  if (this.settings.xProperties === true) {
    ...
    // handle classic format case 
    internals.convertRules(
      property,
      describe.rules,
      ['email', 'ip', 'uri', 'hostname'],
      'format',
      true
    );
    internals.convertRules(
      property,
      describe.rules,
      ['creditCard', 'alphanum', 'token', 'isoDate', 'guid', 'hex'],
      'x-format'
    );
 ...
 * @param  {Boolean} flattenValue
 */
internals.convertRules = function (property, rules, ruleNames, groupName, flattenValue = false) {
  ruleNames.forEach((ruleName) => {
    flattenValue ? internals.appendToPropertyObjectFlatten(property, rules, groupName, ruleName) :
 ...
 * @param  {string} ruleName
 */
internals.appendToPropertyObjectFlatten = function (property, rules, groupName, ruleName) {
  if (internals.hasPropertyByName(rules, ruleName)) {
    property[groupName] = ruleName;
  }
};
