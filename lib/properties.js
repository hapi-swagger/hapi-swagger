...
internals.properties.prototype.parseString = function(property, joiObj) {
  ...
  if (this.settings.xProperties === true) {
    ...
    internals.convertRules(property, describe.rules, ['lowercase', 'uppercase', 'trim'], 'x-convert');
  }
...
internals.properties.prototype.parseObject = function(property, joiObj, name, parameterType, useDefinitions, isAlt) {
  ...
  joiObj._ids._byKey.forEach((key, obj) => {
    let keyName = obj.key;
...
internals.convertRules = function(property, rules, ruleNames, groupName) {
  console.log(rules, ruleNames, groupName)
  ...
  if (Array.isArray(array)) {
    ...
    while (i--) {
      ...
      if (array[i].name === name) {
        if (array[i].options) { return array[i].options }
        console.log(array[i])
        if (array[i].args) {
          console.log(array[i].args)
          console.log(array[i].args.options)
          if (key) {
            return array[i].args[key];
          } else {
            // If not specified, grab first
