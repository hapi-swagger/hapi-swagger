...
properties.parsePropertyMetadata = function (property, joiObj) {
    ...
    property['x-hapi-meta'] = Hoek.reach(joiObj, '_meta.0');
...
internals.parseObject = function (property, joiObj, name, definitionCollection, altDefinitionCollection, isAlt) {
    ...
    if (name) {
        if (isAlt) {
            property.schema = {
                '$ref': '#/x-hapi-definitions/' + require('../lib/definitions').appendJoi(name, joiObj, definitionCollection, altDefinitionCollection, null, true)
            };
        } else {
            property.schema = {
...
internals.parseArray = function (property, joiObj, name, definitionCollection, altDefinitionCollection, type, isAlt) {
    ...
    if (firstInclusionType) {
        ...
        let arrayProperty = properties.parseProperty(name, firstInclusionType, definitionCollection, altDefinitionCollection, null, isAlt);
