    ...
    if (statusSchemas) {
        for (var key in statusSchemas) {
            if (statusSchemas.hasOwnProperty(key)) {
                ...
                //var responseName = Utilities.getJoiMetaProperty(statusSchemas[key], 'name');
                var responseName = Hoek.reach(statusSchemas[key], '_settings.language.label');
                out[key] = {
    ...
    for (var key in userDefindedObjs) {
        if (userDefindedObjs.hasOwnProperty(key)) {
            ...
            if (Hoek.reach(userDefindedObjs[key], 'schema.isJoi') && userDefindedObjs[key].schema.isJoi === true) {
                ...
                var responseName = Hoek.reach(userDefindedObjs[key].schema, '_settings.language.label');
