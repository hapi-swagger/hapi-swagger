...
internals.responses.prototype.getResponse = function (statusCode, joiObj, useDefinitions, isAlt) {
    ...
    /*
    if (Utilities.hasKey(out.schema, 'example')) {
        // the property example is not allowed in a response;
        // do NOT turn into a definition
        out.schema = Utilities.findAndRenameKey(out.schema, 'example', null);
        out.schema = Utilities.findAndRenameKey(out.schema, 'required', null);
    } else {
        if(useDefinitions){
