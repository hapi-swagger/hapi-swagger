...
exports = module.exports = internals.responses = function (settings) {
 ...
 * @return {Object}
 */
internals.responses.prototype.build = function (userDefindedSchemas, defaultSchema, statusSchemas, definitionCollection, altDefinitionCollection, altName) {
    ...
    if (Utilities.hasProperties(statusSchemas)){
        ...
        for (let key in statusSchemas) {
            ...
            out[key] = {
                ...
                'schema': {
                    '$ref': '#/definitions/' + this.definitions.appendJoi(responseName, statusSchemas[key], definitionCollection, altDefinitionCollection, null, false),
                    'title': key
                }
            };
            ...
            delete out[key].schema.type;
            delete out[key].schema.title;
    ...
    if (Utilities.hasProperties(userDefindedSchemas) === true) {
        out = this.override(out, userDefindedSchemas, definitionCollection, altDefinitionCollection, altName);
    }
 ...
 * @return {Object}
 */
internals.responses.prototype.override = function (discoveredSchemas, userDefindedSchemas, definitionCollection, altDefinitionCollection, altName) {
    ...
    for (let key in userDefindedSchemas) {
        ...
        if (Hoek.reach(userDefindedSchemas[key], 'schema.isJoi') && userDefindedSchemas[key].schema.isJoi === true) {
            ...
            let newObj = this.properties.parseProperty(
            ...
            out = internals.formatObject(out, newObj);
            delete out.schema.type;
            delete out.schema.title;
...
/**
 * format object in response structure
 *
 * @param  {Object} currentObj
 * @param  {Object} newObj
 * @return {Object}
 */
internals.formatObject = function ( currentObj, newObj ) {

    if (newObj.schema) {
        currentObj.schema = newObj.schema;
        currentObj.schema.type = newObj.type;
    } else {
        currentObj.schema = newObj;
    }
    return currentObj;
};
