...
internals.paths.prototype.buildRoutes = function(routes) {
  ...
  routes.forEach(route => {
    ...
    if (payloadType.toLowerCase() === 'json') {
      // set as json
      payloadStructures = this.getSwaggerStructures(payloadJoi, 'body', true, false, this.settings.deReferenceArrays);
    } else {
      // set as formData
 ...
 * @param  {Boolean} isAlt
 * @param  {Boolean} deReferenceArrays
 * @return {Object}
 */
internals.paths.prototype.getSwaggerStructures = function (joiObj, parameterType, useDefinitions, isAlt, deReferenceArrays) {
  let outProperties;
  let outParameters;

  if (joiObj) {
    // name, joiObj, parent, parameterType, useDefinitions, isAlt
    outProperties = this.properties.parseProperty(null, joiObj, null, parameterType, useDefinitions, isAlt, deReferenceArrays);
