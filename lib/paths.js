...
internals.paths.prototype.build = function (routes) {
    ...
    routes.forEach((route) => {
        ...
        if (Hoek.reach(routeOptions, 'validate.query')) {
            routeData.queryParams = this.properties.joiObjectToArray(Hoek.reach(routeOptions, 'validate.query'));
        }
        if (Hoek.reach(routeOptions, 'validate.params')) {
            routeData.pathParams = this.properties.joiObjectToArray(Hoek.reach(routeOptions, 'validate.params'));
        }
        if (Hoek.reach(routeOptions, 'validate.headers')) {
            routeData.headerParams = this.properties.joiObjectToArray(Hoek.reach(routeOptions, 'validate.headers'));
        }
...
internals.paths.prototype.buildRoutes = function (routes) {
    ...
    routes.forEach((route) => {
        ...
        if (payloadType.toLowerCase() === 'json') {
            ...
            // set as json
            payloadParameters = this.properties.parseProperty(null, route.payloadParams, swagger.definitions, swagger['x-alt-definitions']);
            ...
            if (payloadParameters && payloadParameters.type !== 'void') {
                ...
                if (!payloadParameters.schema) {

                    let ref = this.definitions.appendJoi(
                        definitionName,
            ...
            payloadParameters = this.properties.toParameters(internals.getJOIObj(route, 'payloadParams'), swagger.definitions, swagger['x-alt-definitions'], 'formData', false);
            // add form mimetype
        ...
        var pathParams = this.properties.toParameters(internals.getJOIObj(route, 'pathParams'), swagger.definitions, swagger['x-alt-definitions'], 'path', false);
        ...
        let headerParams = this.properties.toParameters(internals.getJOIObj(route, 'headerParams'), swagger.definitions, swagger['x-alt-definitions'], 'header', false);
        ...
        out.parameters = out.parameters.concat(
            headerParams,
            pathParams,
            this.properties.toParameters(internals.getJOIObj(route, 'queryParams'), swagger.definitions, swagger['x-alt-definitions'], 'query', false)
            );
