        ...
        [
            ...
            if (Utilities.isFunction(routeData[property])) {
                if (property !== 'pathParams') {
                    ...
                    self.settings.log(['vaildation', 'error'], 'Using Joi.function for a params is not supported and has been removed.');
...
internals.paths.prototype.buildRoutes = function (routes) {
    ...
    //   let xxx = this.properties.parseProperty('x', Joi.array().items({ 'text': Joi.string() }), 'formData', true, false);
    ...
    routes.forEach((route) => {
        ...
        let payloadStructures;
        if (payloadType.toLowerCase() === 'json') {

            // set as json
            payloadStructures = this.getSwaggerStructures(internals.getJOIObj(route, 'payloadParams'), 'body', true, false);
        } else {

            payloadStructures = this.getSwaggerStructures(internals.getJOIObj(route, 'payloadParams'), 'formData', false, false);
            // add form mimetype
        ...
        let pathStructures = this.getSwaggerStructures(internals.getJOIObj(route, 'pathParams'), 'path', false, false);
        pathStructures.parameters.forEach(function (item) {
            ...
            if (!item.required) {
                self.settings.log(['validation', 'warning'], 'A path parameter {' + item.name + '} is set as optional. This will work in the UI, but is invalid in the swagger spec');
            }
        ...
        let headerStructures = this.getSwaggerStructures(internals.getJOIObj(route, 'headerParams'), 'header', false, false);
        ...
        let queryStructures = this.getSwaggerStructures(internals.getJOIObj(route, 'queryParams'), 'query', false, false);
...
internals.paths.prototype.getSwaggerStructures = function (joiObj, parameterType, useDefinitions, isAlt) {
    ...
    if (joiObj) {
        outProperties = this.properties.parseProperty(null, joiObj, parameterType, useDefinitions, isAlt);
        outSchema = Schema.build(outProperties, 'object');
        outParameters = Parameters.fromProperties(outSchema, parameterType);
    }
    let out = {
