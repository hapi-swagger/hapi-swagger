...
paths.build = function( settings, routes ){
    ...
    routes.forEach(function (route) {
        ...
        if( Hoek.reach(routeOptions, 'validate.headers') ){
            routeData.headerParams =  internals.paramObjectToArray( Hoek.reach(routeOptions, 'validate.headers') );
        }
...
paths.properties = function( settings, routes ){
    ...
    routes.forEach(function (route, indexA) {
        ...
        if (payloadType && payloadType.toLowerCase() === 'json') {
            ...
            //console.log(JSON.stringify(route.payloadParams))
            
            var definition  = internals.validatorsToProperties(payloadParam, swagger.definitions);
            
            if(payloadParam !== null){
                payloadApiParams = {
                    "in": "body",
                    "name": "body",
                    "schema":  {'$ref': '#/definitions/' + internals.appendDefinition( out.operationId, definition, swagger.definitions) }
                }
            }
        } else {
            // set as form
        ...
        var responseClassName = internals.getSchemaClassName(route.responseSchema);
        ...
        if(responseClassName){
            ...
            out.responses['200'].schema = {
                "$ref": "#/definitions/" + responseClassName
            }
            
        }
        
       // internals.joiToDefinitionRef = function (name, params, definitions)
...
internals.validatorToProperty = function (name, param, definitions, requiredArray) {
    ...
    if (typeof param.validate !== 'function') {
        property.type = internals.joiToDefinitionRef(name, param, definitions);
        return property;
    }
    ...
    if (param.describe) {
        ...
        if(param._description){
            property.description  = param._description; 
        }
        if(param._notes && param._tags.length > 0){
            property.notes  = param._notes; 
        }
        if(param._tags && param._tags.length > 0){
            property.tags  = param._tags; 
        }
        ...
        if (property.type === 'object' && param._inner) {
            var params = (param._inner.children) ? param._inner.children : param._inner;
            //property.name = name;
            property.type = 'object'
            property.properties = internals.validatorsToProperties(params, definitions, requiredArray);
        }
...
internals.getSchemaClassName = function(schema) {
...
internals.appendDefinition = function (name, params, definitions) {
