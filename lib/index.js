    ...
    apiData.routes.forEach(function(route, indexA) {
        ...
        if (route.pathParams) {
            route.pathParams.forEach(function(param, indexB) {
                api.operations[0].parameters.push(internals.convertParam(param, 'path'));
            });
        }
        if (route.payloadParams) {
            route.payloadParams.forEach(function(param, indexB) {
                api.operations[0].parameters.push(internals.convertParam(param, 'payload'));
            });
        }
        if (route.queryParams) {
            route.queryParams.forEach(function(param, indexB) {
                api.operations[0].parameters.push(internals.convertParam(param, 'query'));
            });
        }
...
internals.convertParam = function(param, type) {
    var p = {
        "name": param.name,
