...
internals.getRoutesData = function(routes) {
    ...
    routes.forEach(function(route) {
        ...
        routesData.push({
            ...
            responseSchema: route.settings.validate && route.settings.validate.response && route.settings.validate.response.schema
        });
    });
...
internals.buildAPIInfo = function(settings, apiData, slug) {
    ...
    apiData.routes.forEach(function(route, indexA) {
        ...
        var responseProperty = internals.validatorToProperty(op.nickname + '_response', route.responseSchema, swagger.models);
        if (responseProperty) {
            op.type = responseProperty.$ref || responseProperty.type || 'void';
            if (op.type !== 'void') {
...
internals.validatorToProperty = function(name, param, models, requiredArray) {
    ...
    if (param.describe) {
        ...
        if (Array.isArray(describe.valids) && describe.valids.length) {
            if (!internals.arrayOnlyContains(describe.valids, [undefined, null])) {
                property["enum"] = describe.valids;
            }
        }
        ...
        if (property.type === 'number') {
            ...
            if (internals.getByName(describe.rules, 'integer')) {
        ...
        if (property.type === 'object' && param._children) {
            property.type = internals.validatorsToModelName(
                param._settings.className || param._settings.typeName,
                param._children,
                models);
        }
        ...
        property.required = param.__modifiers && internals.getByName(param.__modifiers._values, 'required');
...
internals.getByName = function(array, name) {
...
// get arg value of an item in array of structure [ { name: 'name', arg: 'arg' } ]
internals.getArgByName = function(array, name) {
    return array && array.reduce(function(a, v) {
        return v.name === name ? v.arg : a;
    }, undefined);
};
