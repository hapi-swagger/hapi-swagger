...
const schema = Joi.object({
    ...
    swaggerUITemplatePath: Joi.string(),
    swaggerUIRelativePaths: Joi.boolean(),
...
exports.register = function (plugin, options, next) {
    ...
    // set the "swagger ui *template* path" indipendently
    if (settings.swaggerUIRelativePaths === true) {
        // relative swaggerUIPath
        const docRoot = Path.dirname(settings.documentationPath);
        settings.swaggerUITemplatePath = './' + Path.relative(docRoot, settings.swaggerUIPath) + '/';
        settings.jsonTemplatePath = Path.relative(settings.documentationPath, settings.jsonPath);
    }
