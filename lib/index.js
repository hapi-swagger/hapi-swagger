...
internals.validatorToProperty = function (name, param, models, requiredArray, altClassName) {
    if (param === null ||
        param === undefined) {
        return undefined;
    }
    ...
    if (typeof param.validate !== 'function') {
        property.type = internals.validatorsToModelName(name, param, models, altClassName);
        return property;
    }
    ...
    if (param.describe) {
        ...
        if (property.type === 'array') {
            ...
            if (firstInclusionType) {
                ...
                var arrayProperty = internals.validatorToProperty(name, firstInclusionType, models, null, altClassName);
        ...
        if (property.type === 'any') { 
            if(param._settings  
                && param._settings.swaggerType 
                && param._settings.swaggerType == 'file'){ 
                    property.type = "file";  
...
internals.validatorsToModelName = function (name, params, models, altClassName) {
    ...
    if (foundModel) {
        ...
        if(Hoek.deepEqual(foundModel, model)){
            ...
            model.id = altClassName;
            models[altClassName] = model;
        }
    }else{
        // create new model
        models[name] = model;
