...
internals._getClassName = function(schema) {
    return Hoek.reach(schema, '_meta.0.className') ?
        Hoek.reach(schema, '_meta.0.className'): undefined;
}
    ...
    if (param.describe) {
        ...
        if (property.type === 'array') {
            ...
            if (firstInclusionType) {
                ...
                if(name === 'items' && Hoek.reach(param, '_inner.inclusions.0._meta.0.className')){
                    name = Hoek.reach(param, '_inner.inclusions.0._meta.0.className');
                }
        ...
        if (property.type === 'any') { 
           var swaggerType = Hoek.reach(param, '_meta.0.swaggerType');
            if(swaggerType === 'file'){ 
                property.type = "file";  
